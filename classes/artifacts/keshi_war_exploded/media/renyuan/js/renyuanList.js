...
$(function(){
    ...
    getData();
   	...
   	$("#addbutton").on("click",function(){
   		...
   		window.location.href=getIP()+"media/renyuan/renyuanAdd.jsp";
   	})
    ...
    $("#delbutton").on("click",function(){
    	...
    	if(confirm("确定要删除吗？")){
        	...
        	del.ajax(getIP()+"renyuan.action?delMore",function(data){
        		if(data.success){
    ...
    function getData(){  
	   	...
	   	getPageData(sel,getIP()+"renyuan.action?list",function(data){
	   		if(data.success){
	   			var len=data.total;
...
//修改
function update(id){
	window.location.href=getIP()+"media/renyuan/renyuanModify.jsp?eid="+id;
}
